...
# --- Layout principal: formulario a la izquierda y gráficos a la derecha ---
col_form, col_graf = st.columns([1, 3])
...
with col_form:
    with st.form("form_riesgo", clear_on_submit=False):
        ...
        agregar = st.form_submit_button(t["agregar_riesgo"], help="Agregar riesgo", use_container_width=True)
...
if not st.session_state.riesgos.empty:
    ...
    st.dataframe(df.style.apply(
        lambda x: [f"background-color: {x['Color']}" if col=="Color" else "" for col in df.columns], axis=1
    ))
    # Botón descarga Excel verde
    output = BytesIO()
    ...
    st.download_button(
        ...
        help="Descargar matriz en Excel",
        # color verde en botón (Streamlit no soporta color directo, pero en tema claro este botón es azul, aceptable)
    )
else:
    st.info(t["info_agrega_riesgos"])
